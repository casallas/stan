...
if (!exists("anova_pilots_chr.sm")) {
    if (file.exists("anova_pilots_chr.sm.RData")) {
        load("anova_pilots_chr.sm.RData", verbose = TRUE)
    } else {
        rt <- stanc("anova_pilots_chr.stan", model_name = "pilots")
        anova_pilots.sm <- stan_model(stanc_ret = rt)
        save(anova_pilots.sm, file = "anova_pilots_chr.sm.RData")
    }
}
