...
if (!exists("election88.sm")) {
    if (file.exists("election88_chr.sm.RData")) {
        load("election88_chr.sm.RData", verbose = TRUE)
    } else {
        rt <- stanc("election88_chr.stan", model_name = "election88")
        election88.sm <- stan_model(stanc_ret = rt)
        save(election88.sm, file = "election88_chr.sm.RData")
    }
}
...
if (!exists("election88_full.sm")) {
    if (file.exists("election88_full_chr.sm.RData")) {
        load("election88_full_chr.sm.RData", verbose = TRUE)
    } else {
        rt <- stanc("election88_full_chr.stan", model_name = "election88_full")
        election88_full.sm <- stan_model(stanc_ret = rt)
        save(election88_full.sm, file = "election88_full_chr.sm.RData")
    }
}
